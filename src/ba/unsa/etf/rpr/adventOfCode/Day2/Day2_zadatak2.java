package ba.unsa.etf.rpr.adventOfCode.Day2;

import java.util.Arrays;
import java.util.List;

public class Day2_zadatak2 {
    static boolean validno(int start, int end, String slovo, String x){
        int repeat = 0;
        if(x.charAt(start) == slovo.charAt(0)) repeat++;
        if(x.charAt(end) == slovo.charAt(0)) repeat++;
        if(repeat != 1) return false;
        return true;
    }

    static String input() {
        String input = "1-7 q: qqqqxvqrkbqqztlqlzq\n" +
                "1-3 q: cqbm\n" +
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i<lista.size();i++) {
            ...
            String[] split2 = parts[0].split(" ");
            String slovo = split2[1];

            String[] split3 = split2[0].split("-");
            int start = Integer.parseInt(split3[0]);
            int end = Integer.parseInt(split3[1]);
            if(validno(start, end, slovo, password)) validPasswords++;
