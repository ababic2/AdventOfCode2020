package ba.unsa.etf.rpr.adventOfCode.Day4;
...
public class Day4_Zadatak2 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < strArray.length; i++) {
            if(!strArray[i].trim().isEmpty()) { ///nije blank string
                //ukoliko nije blank razdvoji po " " i onda broji
                // ukoliko jest blank provjeri stanje polja i update validan brojac
                String[] line = strArray[i].split( " ");
                for(int j =0; j < line.length; j++) {
//                    System.out.println("HERE I AM " + line[j]);
                    String[] fields = line[j].split(":");
                    mapa.put(fields[0], fields[1]);
//                    System.out.println(fields[0]+ " " + fields[1]);
                }

            }
            if(strArray[i].trim().isEmpty() || i == strArray.length - 1){
                //svaki pasos provjeri
                ...
                if(checkMapValidation(mapa))
                    valid++;

                mapa = new HashMap<String, String>();
            }
        }
//        System.out.println("for" + strArray[2] + "hehe");
        System.out.println(valid);
    }

    private static boolean checkIfMapContainsKeys(Map<String, String> mapa) {
        if(mapa.containsKey("byr") && mapa.containsKey("iyr") &&
                mapa.containsKey("eyr") && mapa.containsKey("hgt") &&
                mapa.containsKey("hcl") && mapa.containsKey("ecl") &&
                mapa.containsKey("pid") )
            return true;
        return false;
    }

    private static boolean checkMapValidation(Map<String, String> mapa) {
        if(!checkIfMapContainsKeys(mapa)) return false;
        else {
            Integer byr = Integer.parseInt(mapa.get("byr"));
            Integer iyr = Integer.parseInt(mapa.get("iyr"));
            Integer eyr = Integer.parseInt(mapa.get("eyr"));
            String hgt = mapa.get("hgt");
            String hcl = mapa.get("hcl");
            String ecl = mapa.get("ecl");
            String pid = mapa.get("pid");
            if(checkYears(byr, iyr, eyr) &&
                    checkHeight(hgt) &&
                    checkHairColor(hcl) &&
                    checkEyeColor(ecl) &&
                    checkPID(pid)) return true;
        }
        return false;
    }
...
//    If in, the number must be at least 59 and at most 76.
    private static boolean checkHeight(String hgt) {
        ...
        {
            if(hgt.charAt(i) == 'c' || hgt.charAt(i) == 'i') {
                tip += hgt.charAt(i);
                break;
            }
            broj += hgt.charAt(i);
        ...
        if(tip.equals("c")) {
            if(Integer.parseInt(broj) >= 150 && Integer.parseInt(broj) <= 193) return true;
        } else if(tip.equals("i")) {
            if(Integer.parseInt(broj) >= 59 && Integer.parseInt(broj) <= 76) return true;
        }
        return false;
    ...
    private static boolean checkHairColor(String hcl) {
        ...
        return hcl.length() == 7 && Pattern.matches(pattern, hcl);
    }
    ...
    private static boolean checkEyeColor(String ecl) {
        ...
        if(ecl.equals("amb") || ecl.equals("blu") || ecl.equals("brn") ||
                ...
                ecl.equals("oth")) return true;
        return false;
    }

    private static boolean checkYears(Integer byr, Integer iyr, Integer eyr) {
        if((byr >= 1920 && byr <= 2002) && (iyr >= 2010 && iyr <= 2020) && (eyr >= 2020 && eyr <= 2030)) return true;
        return false;
    }
